...
# - s_final = p_cls + eta * relu(s_mem - p_cls)
# - Binarization with a single global delta (no additional closure)
# - Batch inference utilities and ef_search control
...
class InferenceConfig:
    eta: float = 0.5           # memory boost strength over classifier scores
...
class InferenceEngine:
    ...
    def fuse_scores(self, s_mem: Tensor, p_cls: Tensor, eta: Optional[float] = None) -> Tensor:
        """
        Non-symmetric fusion where memory only boosts classifier scores:
          s = p_cls + eta * relu(s_mem - p_cls)
        If memory is lower than the classifier on a label, it does not suppress it.
        """
        ...
        boost = torch.relu(s_mem - p_cls)
        s = (p_cls + eta_val * boost).clamp(0.0, 1.0)
        return s

    # --- Binarization ---
    def binarize(self, scores: Tensor, delta: Optional[float] = None) -> Tensor:
        """
        Apply global delta to produce {0,1}.
        """
        delta_val = self.cfg.delta if delta is None else float(delta)
        return (scores >= delta_val).to(torch.int64)
    ...
    def predict_single(
        ...
        out = self.predict_batch(s_mem, p_cls, eta=eta, delta=delta, return_intermediate=True)
...
if __name__ == "__main__":
    ...
    cfg = InferenceConfig(eta=0.6, delta=0.5, device=device)
