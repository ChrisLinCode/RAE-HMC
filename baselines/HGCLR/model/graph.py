    ...
    def __init__(self, config, graph=False, layer=1, data_path=None, threshold=0.01, tau=1,
        ...
        if graph:
            ...
            if GRAPH == 'GRAPHORMER':
                ...
                label_count = len(self.inverse_label_list)
                self.distance_mat = torch.zeros(label_count, label_count, dtype=torch.long)
                for i in range(label_count):
                    for j in range(label_count):
                        self.distance_mat[i, j] = get_distance(i, j)
                self.distance_mat = self.distance_mat.view(1, -1)
                self.edge_mat = torch.zeros(label_count, label_count, 15, dtype=torch.long)
                for i in range(label_count):
                    for j in range(label_count):
