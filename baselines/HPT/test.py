...
if __name__ == '__main__':
    ...
    if args.model == 'prompt':
        if os.path.exists(os.path.join(data_path, args.model)):
            ...
            if len(prefix) >= args.max_token:
                raise ValueError(
                    "Prompt prefix length {} must be smaller than --max-token {}.".format(len(prefix), args.max_token)
                )
            ...
            def data_map_function(batch, tokenizer):
                ...
                for l, t in zip(batch['label'], batch['token']):
                    ...
                    new_batch['input_ids'].append(tokens['input_ids'][:-1][:args.max_token - len(prefix)] + prefix)
                    new_batch['input_ids'][-1].extend(
                        [tokenizer.pad_token_id] * (args.max_token - len(new_batch['input_ids'][-1])))
                    new_batch['attention_mask'].append(
                        tokens['attention_mask'][:-1][:args.max_token - len(prefix)] + [1] * len(prefix))
                    new_batch['attention_mask'][-1].extend([0] * (args.max_token - len(new_batch['attention_mask'][-1])))
                    new_batch['token_type_ids'].append([0] * args.max_token)
    ...
    path_total = father_false + father_count
    path_acc = father_count / path_total if path_total > 0 else 0.0
    print('path acc', path_acc)
