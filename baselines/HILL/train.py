...
def _load_rows(path_prefix):
    pt_path = Path(path_prefix + '.pt')
    if pt_path.exists():
        return torch.load(pt_path, map_location='cpu')

    txt_path = Path(path_prefix + '.txt')
    if txt_path.exists():
        rows = []
        with txt_path.open('r', encoding='utf-8') as handle:
            for line in handle:
                line = line.strip()
                rows.append([int(token) for token in line.split()] if line else [])
        return rows

    raise FileNotFoundError('Could not find {}.pt or {}.txt'.format(path_prefix, path_prefix))
...
def resolve_max_token(data_path, requested, default=512):
    ...
    if meta_path.exists():
        ...
        recommended = meta.get('max_token_recommended')
...
class BertDataset(Dataset):
    def __init__(self, max_token=512, device='cpu', pad_idx=0, data_path=None):
        ...
        self.data = _load_rows(data_path + '/tok')
        self.labels = _load_rows(data_path + '/Y')
    ...
    def __getitem__(self, item):
        ...
        data = data[:self.max_token].to(self.device)
...
def run_test(config, test_set, extra):
    checkpoint = torch.load(
        os.path.join(config.ckpt_dir, config.name, 'best_{}.pt'.format(extra)),
        map_location='cpu',
        weights_only=False,
    )
    model = MODELS[config.model_name].from_pretrained(bert_file, num_labels=num_class, local_config=config)
...
if __name__ == '__main__':
    ...
    if args.epochs is not None:
        config.train.epoch = int(args.epochs)
    if args.early_stop is not None:
        config.train.early_stop = int(args.early_stop)
    config.device_setting.device = args.device
