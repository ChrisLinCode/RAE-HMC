...
class GraphEncoder(nn.Module):
    def __init__(self, config, graph=False, layer=1, data_path=None, threshold=0.01, tau=1):
        ...
        self.label_dict = {i: self.tokenizer.decode(v, skip_special_tokens=True) for i, v in self.label_dict.items()}
        self.label_name = []
        ...
        if graph:
            ...
            if GRAPH == 'GRAPHORMER':
                ...
                self.distance_mat = torch.zeros(
                    self.label_id.size(0), self.label_id.size(0), dtype=torch.long
                )
                for src_idx in range(self.label_id.size(0)):
                    for dst_idx in range(self.label_id.size(0)):
                        self.distance_mat[src_idx, dst_idx] = get_distance(src_idx, dst_idx)
